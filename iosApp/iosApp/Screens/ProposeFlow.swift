...
struct BoulderFormSheet: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    ForEach($blocks) { $b in blockRow($b) }
    ...
    @ViewBuilder private func blockRow(_ b: Binding<BoulderBlockForm>) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                TextField("Nombre (opcional)", text: b.name).font(.system(size: 14))
                    .padding(8).overlay(Rectangle().stroke(Cumbre.rule, lineWidth: 1))
                if blocks.count > 1 {
                    let bid = b.wrappedValue.id
                    Button { blocks.removeAll { $0.id == bid } } label: {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 6) {
                    ForEach(BOULDER_GRADES, id: \.self) { g in
                        let on = b.grade.wrappedValue == g
                        Button { b.grade.wrappedValue = on ? nil : g } label: {
            ...
            HStack(spacing: 6) {
                ForEach(START_TYPES, id: \.self) { st in
                    let on = b.startType.wrappedValue == st
                    Button { b.startType.wrappedValue = on ? nil : st } label: {
                ...
                if !b.line.wrappedValue.isEmpty {
                    Text("✓ línea (\(b.line.wrappedValue.count))").font(Cumbre.mono(10)).foregroundStyle(Cumbre.ok)
                }
            }
        }
        .padding(10).overlay(Rectangle().stroke(Cumbre.rule, lineWidth: 1))
