...
final class GripScaleBridge: NSObject, IosGripScaleBridge, CBCentralManagerDelegate {
    ...
    private var onWeightReading: ((KotlinDouble, KotlinDouble) -> Void)?
    ...
    func observeWeight(deviceId: String, onReading: @escaping (KotlinDouble, KotlinDouble) -> Void) -> IosGripListener {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
        ...
        if observingDeviceId == deviceId, onWeightReading != nil, payload.count >= 12 {
            ...
            onWeightReading?(KotlinDouble(double: kg), KotlinDouble(double: timestampMs))
        }
    }
}
