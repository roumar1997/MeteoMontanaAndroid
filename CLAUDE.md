...
**2026-07-09** — Compartir vía como IMAGEN (Android): al tocar el icono de compartir de una vía se genera un PNG 1080×1920 (formato historia) con la FOTO real de la cara + sus líneas dibujadas (reusa `renderTopo`/`gradeArgb`/`parseLineStroke` + `DrawOp` sobre `Canvas` nativo) y cabecera Cumbre (nombre/grado/piedra·escuela). Se comparte por el share sheet normal (`image/png`) → Instagram (→Historia), WhatsApp, etc., igual que las escuelas. `ShareLineImage.kt` (`shareLineAsImage` suspend, descarga foto con Coil); `BlockDetailDialog.shareVia` orquesta imagen→fallback texto si la vía no tiene foto/dibujo. PENDIENTE (opcional): botón directo a Instagram Stories (`com.instagram.share.ADD_TO_STORY`) requiere un Facebook App ID registrado; paridad iOS.
